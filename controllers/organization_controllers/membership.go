...
func AddMember(c *fiber.Ctx) error {
	...
	if err := initializers.DB.Where("user_id=? AND organization_id=?", user.ID, parsedOrganizationID).First(&membership).Error; err != nil {
		if err == gorm.ErrRecordNotFound {
			...
			go routines.MarkOrganizationHistory(parsedOrganizationID, parsedUserID, 3, nil, nil, nil, nil, &invitation.ID, nil, nil, "")
...
func RemoveMember(c *fiber.Ctx) error {
	...
	go routines.MarkOrganizationHistory(membership.OrganizationID, parsedOrgMemberID, 5, nil, nil, nil, nil, nil, nil, nil, membership.Title)
...
func LeaveOrganization(c *fiber.Ctx) error {
	...
	go routines.DecrementOrgMember(membership.OrganizationID)
	go routines.MarkOrganizationHistory(membership.OrganizationID, parsedOrgMemberID, 15, nil, nil, nil, nil, nil, nil, nil, membership.Title)
	go cache.RemoveOrganization("-access--" + membership.OrganizationID.String())
