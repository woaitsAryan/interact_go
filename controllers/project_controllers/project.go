		...
		if err := initializers.DB.Where("slug=?", newSlug).First(&existingProject).Error; err != nil {
			if err == gorm.ErrRecordNotFound {
				...
				if orgMemberID != "" && orgID != "" {
					...
					go routines.MarkOrganizationHistory(parsedOrgID, parsedOrgMemberID, 9, nil, &newProject.ID, nil, nil, nil, nil, nil, nil, "")
...
func UpdateProject(c *fiber.Ctx) error {
	...
	if orgMemberID != "" {
		...
		go routines.MarkOrganizationHistory(parsedOrgID, parsedOrgMemberID, 11, nil, &project.ID, nil, nil, nil, nil, nil, nil, "")
	} else {
