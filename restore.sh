...
if [ -f /path/to/.env.db ]; then
    ...
    echo "$(date '+%Y-%m-%d %H:%M:%S') ERROR: .env.db file not found." >> ./log/backup.log
    exit 1
fi
...
if [ -z "$MOST_RECENT_BACKUP" ]; then
    echo "$(date '+%Y-%m-%d %H:%M:%S') ERROR: No backup files found in $BACKUP_DIR." >> ./log/backup.log
    exit 1
fi
...
if PGPASSWORD="$DB_PASSWORD" pg_restore -h "$DB_HOST" -U "$DB_USER" -d "$DB_NAME" < "$BACKUP_FILE"; then
    echo "$(date '+%Y-%m-%d %H:%M:%S') INFO: Database restore completed successfully from $MOST_RECENT_BACKUP." >> ./log/backup.log
else
    echo "$(date '+%Y-%m-%d %H:%M:%S') ERROR: Database restore failed. Please check for errors." >> ./log/backup.log
    exit 1
fi
